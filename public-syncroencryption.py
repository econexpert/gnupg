...
class App(tk.Tk):
  def __init__(self):
    super().__init__()
    print("testing for GnuPG keys present in the system.")
    self.gpg = gnupg.GPG()
    if not self.gpg:
       print("GnuPG is not installed.")
    else:
    # Check if a GPG key is present
        self.keys = self.gpg.list_keys()
    if not self.keys:
        print("No GPG keys are present.")
...
#            self.file_list.delete(0, tk.END)
  ...
  def encrypt_file(self):
    ...
    # Initialize GNUPG and set path to private key
    gpg = gnupg.GPG(gnupghome=os.path.expanduser('~/.gnupg'))

    # Encrypt file using signer's private key
    with open(self.file_path, 'rb') as f:
        encrypted_data = gpg.encrypt_file(f, recipients=None, symmetric="AES256", passphrase=self.recipient_entry.get())  # encryption part
        print("date encyprion status:", encrypted_data.status)
    ...
    if encrypted_data.ok:
        ...
        tk.messagebox.showinfo('Error',encrypted_data.status)
        return
...
# -----  continue with data upload
  ...
  def decrypt_file(self):
    ...
    with open(self.file_path, 'rb') as f:
        self.decrypted_data = self.gpg.decrypt_file(f, passphrase=self.passphrase)
        print(self.decrypted_data.status)
        if self.decrypted_data.ok:
            print("decrypted status is ok")
        else:
            tk.messagebox.showerror('Failed','Decryption failed!')
            return
    ...
    with open(self.file_path, 'wb') as f:
        f.write(self.decrypted_data.data)
